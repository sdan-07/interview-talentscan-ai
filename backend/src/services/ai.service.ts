...
export const generateInterviewReport = async ({ resume, selfDescription, jobDescription }:generateReportType, retries: number = 3 ): Promise<z.infer<typeof interviewReportSchema>> => {
  ...
  throw new Error("Failed to generate interview report after all retry attempts.");
};
